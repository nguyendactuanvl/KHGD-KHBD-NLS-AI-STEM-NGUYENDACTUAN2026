...
if (!code.includes('Loader2')) {
  code = code.replace('Upload, Plus, Save, Trash2, Award, FileText', 'Upload, Plus, Save, Trash2, Award, FileText, Loader2, FileImage, Search, Edit3');
}
...
`;
code = code.replace('const [activeTab, setActiveTab] = useState<"list" | "competition">("list");', 'const [activeTab, setActiveTab] = useState<"list" | "competition">("list");\n' + newStates);
...
`;
code = code.replace('return (', filteredLogic + '\n  return (');
...
code = code.replace(/const handleFileUpload = \(e: React.ChangeEvent<HTMLInputElement>\) => \{[\s\S]*?reader.readAsText\(file\);\s*\};/g, newHandleUpload.trim());
...
code = code.replace(/<div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">[\s\S]*?<\/div>\s*<\/div>/, newHeader.trim());
...
code = code.replace(/<div className="flex-1 overflow-auto p-0">\s*<table[\s\S]*?<\/div>/, newTable.trim());


fs.writeFileSync('src/pages/HomeroomManagement.tsx', code);
console.log('Homeroom patched');
