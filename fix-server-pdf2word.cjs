...
2. CHUYỂN TOÀN BỘ CÔNG THỨC Toán học, Vật lý, Hóa học sang định dạng chuẩn LaTeX:
...
const insertPos = code.indexOf('app.post("/api/solve-exercise"');
if (insertPos !== -1 && !code.includes('/api/pdf-to-word')) {
  code = code.substring(0, insertPos) + generateRoute + "\n" + code.substring(insertPos);
  fs.writeFileSync('server.ts', code);
  console.log("Updated server.ts with /api/pdf-to-word");
}
