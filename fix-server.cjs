...
// Find the end of "/api/generate-lesson-plan-file"
const endOfFileGen = 'res.status(500).json({ error: "Failed to generate lesson plan from file" });\n    }\n  });';
const circularsStart = '  // API route to check circulars';

if (code.includes(endOfFileGen)) {
    const firstPart = code.substring(0, code.indexOf(endOfFileGen) + endOfFileGen.length);
    
    // Check where the correct circular route starts
    const lastPartIndex = code.lastIndexOf(circularsStart);
    let lastPart = "";
    if (lastPartIndex !== -1) {
        lastPart = code.substring(lastPartIndex);
    } else {
        lastPart = `
...
`;
    ...
    code = firstPart + apiCode + lastPart;
    fs.writeFileSync('server.ts', code);
    console.log("Success fixed server.ts");
}
