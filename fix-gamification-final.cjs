...
// 1. Add askAi function before the first return (
const askAiStr = `
  const askAi = async () => {
    ...
    setAiResponse("");
    try {
      const rankedData = getRankedStudents().map((st, i) => ({
        rank: i + 1,
        name: st.name,
        score: st.totalScore
      })).slice(0, 15);
...
`;
if (!code.includes('const askAi =')) {
   code = code.replace('return (', askAiStr + '\n  return (');
}
...
const buttonsToInject = `
              <div className="flex w-full mt-2 sm:mt-0 gap-2 flex-wrap">
                ...
                <button onClick={clearAllStudents} className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-600 border border-red-200 text-sm font-medium rounded-md hover:bg-red-100 transition-colors whitespace-nowrap">
...
console.log('Fixed Gamification');
